...
CONFIG_DIR="k8s"
SECRET_NAME="notification-service-secrets" # pragma: allowlist secret
...
print_separator "="
echo -e "${CYAN}🟢 Building Django Docker image: ${FULL_IMAGE_NAME} (inside Minikube Docker daemon)${NC}"
print_separator '-'
...
print_separator "="
echo -e "${CYAN}📦 Deploying Notification Service container...${NC}"
print_separator "-"

kubectl apply -f "${CONFIG_DIR}/deployment.yaml"

print_separator "="
echo -e "${CYAN}🌐 Exposing Notification Service via ClusterIP Service...${NC}"
print_separator "-"
...
if grep -q "notification-service.local" /etc/hosts; then
  echo -e "${YELLOW}🔄 Updating /etc/hosts for notification-service.local...${NC}"
  sed -i "/notification-service.local/d" /etc/hosts
else
  echo -e "${YELLOW}➕ Adding notification-service.local to /etc/hosts...${NC}"
fi
echo "$MINIKUBE_IP notification-service.local" | tee -a /etc/hosts
...
echo "  Health Check: http://notification-service.local/api/v1/notification/health/"
