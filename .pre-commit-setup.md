...
- ✅ **Automatic code formatting** - black and isort format your code
...
- **flake8** - Style guide enforcement
  - Checks: PEP 8, docstrings, complexity
  - Max complexity: 10
  - Plugins: django, bugbear, comprehensions, simplify
...
django-settings-module = "notification_service.settings"
```
...
**Cause:** Our configuration should prevent this, but if it happens...

**Solution:**
```bash
# Black is the authority on formatting
poetry run black .

# Flake8 is configured to be black-compatible
# Update .flake8 or .pre-commit-config.yaml if needed
```
...
# Example: Debug flake8
poetry run pre-commit run flake8 --verbose --all-files
```
...
Black and isort may reformat your code:
...
3. **Skip slow hooks in emergency** - `SKIP=pylint git commit ...`
...
# Skip specific hooks
SKIP=pylint,mypy git commit -m "..."

# Skip only slow hooks
SKIP=mypy,osv-scanner git commit -m "..."
```
