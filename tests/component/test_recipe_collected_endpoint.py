"""Component tests for recipe-collected notification endpoint.

This module tests the /notifications/recipe-collected endpoint through the
...
class TestRecipeCollectedEndpoint(TestCase):
    """Component tests for recipe collected notification endpoint."""

    def setUp(self):
        """Set up test fixtures."""
        self.client = Client()
        self.url = "/api/v1/notification/notifications/recipe-collected"

        # Test data
        self.recipe_id = 123
        self.collection_id = 456
        self.collector_id = uuid4()
    ...
    def test_post_with_user_scope_and_non_follower_sends_anonymous(
        ...
        mock_recipe_client.get_recipe.return_value = self.mock_recipe
        mock_recipe_client.get_collection.return_value = self.mock_collection
        # Collector doesn't follow author - should send anonymous notification
    ...
    def test_post_with_invalid_payload_returns_400(
        ...
        # Invalid payload - missing collection_id
        invalid_data = {
            "recipe_id": self.recipe_id,
            "collector_id": str(self.collector_id),
    ...
    @patch("core.auth.oauth2.OAuth2Authentication.authenticate")
    @patch("core.services.social_notification_service.recipe_management_service_client")
    def test_post_with_nonexistent_collection_returns_404(
        self, mock_recipe_client, mock_authenticate, mock_get_current_user
    ):
        """Test POST with nonexistent collection returns HTTP 404."""
        # Setup authentication
        admin_user = OAuth2User(
            user_id=str(uuid4()),
            client_id="test-client",
