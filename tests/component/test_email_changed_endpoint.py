"""Component tests for email-changed notification endpoint.

This module tests the /notifications/email-changed endpoint through the
...
class TestEmailChangedEndpoint(TestCase):
    """Component tests for email changed notification endpoint."""

    def setUp(self):
        """Set up test fixtures."""
        self.client = Client()
        self.url = "/api/v1/notification/notifications/email-changed"
    ...
    def test_post_with_service_to_service_auth_returns_202(
        ...
        data = response.json()
        self.assertEqual(data["queued_count"], 2)  # Both old and new email
    ...
    def test_post_with_invalid_payload_returns_400(
        ...
        # Invalid payload - missing new_email
        invalid_data = {
    ...
    @patch("core.auth.context.get_current_user")
    @patch("core.auth.oauth2.OAuth2Authentication.authenticate")
    def test_post_with_invalid_email_format_returns_400(
        self, mock_authenticate, mock_get_current_user
    ):
        """Test POST with invalid email format returns HTTP 400."""
        # Setup service-to-service authentication
        service_user_id = str(uuid4())
        service_user = OAuth2User(
            user_id=service_user_id,
        ...
        invalid_data = {
            "recipient_ids": [str(uuid4())],
            "old_email": "invalid-email",  # Not a valid email
    ...
    def test_post_with_multiple_recipients_returns_400(
        ...
        """Test POST with more than 1 recipient returns HTTP 400."""
        # Setup service-to-service authentication
        service_user_id = str(uuid4())
        service_user = OAuth2User(
            user_id=service_user_id,
        ...
        # Invalid payload - multiple recipients (should be exactly 1)
        invalid_data = {
    ...
    def test_response_contains_two_notification_ids(
        ...
        """Test response contains 2 notification IDs (old + new email)."""
        # Setup service-to-service authentication
        service_user_id = str(uuid4())
        service_user = OAuth2User(
            user_id=service_user_id,
