"""Component tests for recipe-featured notification endpoint.

This module tests the /notifications/recipe-featured endpoint through the
...
class TestRecipeFeaturedEndpoint(TestCase):
    """Component tests for recipe featured notification endpoint."""

    def setUp(self):
        """Set up test fixtures."""
        self.client = Client()
        self.url = "/api/v1/notification/notifications/recipe-featured"
    ...
    @patch("core.services.recipe_notification_service.user_client")
    @patch("core.services.recipe_notification_service.notification_service")
    def test_post_without_featured_reason_succeeds(
        self,
        mock_notification_service,
        mock_user_client,
        mock_recipe_client,
        ...
        """Test POST without featured_reason (optional field) succeeds."""
        # Setup authentication
        admin_user = OAuth2User(
            user_id=str(uuid4()),
            client_id="test-client",
        ...
        # Request without featured_reason
        request_data = {
            "recipe_id": self.recipe_id,
            "recipient_ids": [str(rid) for rid in self.recipient_ids],
        }
