"""Component tests for recipe-liked notification endpoint.

This module tests the /notifications/recipe-liked endpoint through the
...
class TestRecipeLikedEndpoint(TestCase):
    """Component tests for recipe liked notification endpoint."""
    ...
    def test_post_with_invalid_follower_returns_403(
        ...
        """Test POST with liker not following author returns HTTP 403."""
        # Setup authentication
        regular_user = OAuth2User(
            user_id=str(uuid4()),
            client_id="test-client",
    ...
    def test_post_with_invalid_payload_returns_400(
        ...
        # Invalid payload - missing liker_id
        invalid_data = {
            "recipe_id": str(uuid4()),
    ...
    @patch("core.services.recipe_notification_service.recipe_management_service_client")
    @patch("core.services.recipe_notification_service.user_client")
    def test_post_with_nonexistent_liker_returns_404(
        self,
        mock_user_client,
        mock_recipe_client,
        mock_authenticate,
        ...
        """Test POST with nonexistent liker returns HTTP 404."""
        # Setup authentication
        admin_user = OAuth2User(
            user_id=str(uuid4()),
            client_id="test-client",
        ...
        # Setup service mocks
        mock_recipe_client.get_recipe.return_value = self.mock_recipe
        # User client raises UserNotFoundError when fetching liker
        mock_user_client.get_user.side_effect = UserNotFoundError(
            user_id=str(self.liker_id)
        )
