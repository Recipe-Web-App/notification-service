"""Tests for RecipeNotificationService recipe-liked with two-table schema."""
...
@pytest.fixture
def recipe_liked_request():
    """Create a recipe liked request."""
...
def test_send_notifications_admin_scope_success(
    ...
    response = recipe_notification_service.send_recipe_liked_notifications(
...
def test_send_notifications_includes_notification_data(
    ...
    assert notification_data["recipe_id"] == str(recipe_liked_request.recipe_id)
    assert notification_data["liker_id"] == str(recipe_liked_request.liker_id)
...
def test_send_notifications_uses_correct_category(
    ...
    call_kwargs = mock_notification_service.create_notification.call_args[1]
    assert call_kwargs["notification_category"] == "RECIPE_LIKED"
